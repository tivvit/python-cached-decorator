...
class Cached(object):
    ...
    def __call__(self, f):
        ...
        def wrapped_f(*args, **kwargs):
            self.name_params = self.name + "_" + str(hash(str(args) + "|" + str(kwargs)))
            ...
            if self.force_run or not return_value:
                ...
                out_file_name = self.name_params + "_" + str(time.time()) + ".pkl"
                with open(os.path.join(self.output_path, out_file_name), "wb+") as out_file:
    ...
    def __load(self):
        ...
        files = glob.glob(os.path.join(self.output_path, self.name_params + "*"))
        if files:
            newest_file = max(files)
            with open(newest_file, "rb") as out:
                from datetime import datetime

                create_time = float(newest_file.replace(".pkl", "").split("_")[-1])
                created = datetime.fromtimestamp(create_time).strftime("%d.%m.%Y %H:%M:%S")
                print "loading %s (created %s)" % (newest_file, created)
                return pickle.load(out)
        else:
            print "%s_*.pkl not found (maybe different args). RUNNING" % os.path.join(self.output_path,
                                                                                      self.name_params)
